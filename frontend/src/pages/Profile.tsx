...
const Profile = () => {
  ...
  const handleSave = async () => {
    ...
    if (profileData?.fullname === fullName) return;
    if (profileData?.fullname.trimEnd().length === fullName.length) return;
    ...
    try {
      ...
      const errorMessage =
        ...
        'Something went wrong during update user details';
  ...
  const handlePasswordChange = async () => {
    ...
    if (!passwordRegex.test(passwordData.newPassword)) {
      return toast.error(
        'Password should be minimum 8 characters with at least 1 uppercase, 1 lowercase, 1 digit, and 1 special character.',
      );
    }
    ...
    try {
      ...
      const errorMessage =
        axiosError.response?.data.message ||
        'Error during chnaging the password';
      toast.error(errorMessage);
    } finally {
              ...
              <div className="flex flex-col items-center mb-8">
                ...
                <p className="text-sm text-gray-500 mt-1">
                  {profileData?.role.charAt(0).toUpperCase() +
                    profileData?.role.slice(1, profileData?.role.length)}
                </p>
              </div>
                ...
                <div className="flex gap-3 mt-6">
                  <Button
                    ...
                    disabled={
                      ...
                      ].some((password) => password.trim().length === 0) ||
                      isChangingPassword
                    }
